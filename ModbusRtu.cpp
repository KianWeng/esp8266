 ...
 * @ingroup setup
 */
 void Modbus::begin(long u32speed, SerialConfig u8config)
{
...
/**
 * Set modbus master state
 *
 * @param uint8_t state  0 IDLE, 1 WAITING FOR ANSWER
 * @ingroup buffer
 */
...
int8_t Modbus::poll()
{
    ...
    if ((unsigned long)(millis() -u32timeOut) > (unsigned long)u16timeOut)
    {
        ...
        u16errCnt++;
        //MBOX_DEBUG_PRINTLN("modbus timeout");
        return 0;
    }
	//else
	//{
	//	MBOX_DEBUG_PRINTLN("[Poll] current time:" + String(millis())+"u32timeOut:" + String(u32timeOut) + "u16timeOut:" + String(u16timeOut));
	//}

    //MBOX_DEBUG_PRINTLN("u8current = " + String(u8current));

    if (u8current == 0) return 0;
 
    //MBOX_DEBUG_PRINTLN("u8current != 0 port available, u8current = " + String(u8current));

    // check T35 after frame end or still no frame end
    if (u8current != u8lastRec)
    {
        //MBOX_DEBUG_PRINTLN("u8current = " + String(u8current) + " u8lastRec = " + String(u8lastRec));
        u8lastRec = u8current;
        u32time = millis();
        return 0;
    }

    //MBOX_DEBUG_PRINT("millis() = " + String(millis()) + "u32time = " + String(u32time) + "millis() -u32time == " + String(millis() - u32time));
    if ((unsigned long)(millis() -u32time) < (unsigned long)T35) return 0;

    //MBOX_DEBUG_PRINTLN("millis() -u32time > 5");

    // transfer Serial buffer frame to auBuffer
    u8lastRec = 0;
    int8_t i8state = getRxBuffer();

    //MBOX_DEBUG_PRINT("i8state = " + String(i8state));
    //MBOX_DEBUG_PRINT("\n");
    ...
    //MBOX_DEBUG_PRINTLN("u8exception = " + String(u8exception));
    //MBOX_DEBUG_PRINTLN("au8Buffer[ FUNC ] = " + String(au8Buffer[ FUNC ]));
    ...
    case MB_FC_READ_REGISTERS: 
        ...
        //MBOX_DEBUG_PRINTLN("get_FC3() start");
        get_FC3( );
        break;
    case MB_FC_WRITE_COIL:
    case MB_FC_WRITE_REGISTER :
...
int8_t Modbus::getRxBuffer()
{
    ...
    //MBOX_DEBUG_PRINTLN("Modbus::getRxBuffer start");
    //for(int i = 0; i < 15; i++){
    //    MBOX_DEBUG_PRINT(au8Buffer[i], HEX)
	//	MBOX_DEBUG_PRINT(" ")
    //}
    //MBOX_DEBUG_PRINTLN("Modbus::getRxBuffer end");
    return u8BufferSize;
}
...
void Modbus::sendTxBuffer()
{
	...
	//MBOX_DEBUG_PRINTLN("Modbus::sendTxBuffer start");
    //for(int i = 0; i < u8BufferSize; i++){
    //    MBOX_DEBUG_PRINT(au8Buffer[i], HEX)
	//	MBOX_DEBUG_PRINT(" ")
    //}
    //MBOX_DEBUG_PRINTLN("Modbus::sendTxBuffer end");
...
void Modbus::get_FC3()
{
    ...
    for (i=0; i< au8Buffer[ 2 ] /2; i++)
    {
        au16regs[ i ] = word(
                            au8Buffer[ u8byte ],
                            au8Buffer[ u8byte +1 ]);
        //MBOX_DEBUG_PRINTLN("au16regs[" + String(i) + "] = " + String(au16regs[ i ]));
        u8byte += 2;
    }
}

/**
 * This method processes functions 6 (for master)
 * This method puts the slave answer into master data buffer
 *
 * @ingroup register
 */
void Modbus::get_FC6()
{
	//MBOX_DEBUG_PRINTLN("au8Buffer[5] = " + String(au8Buffer[5]));
	au16regs[0] = word(au8Buffer[4],au8Buffer[5]);
	//MBOX_DEBUG_PRINTLN("au16regs[0] = " + String(au16regs[0]));
}
